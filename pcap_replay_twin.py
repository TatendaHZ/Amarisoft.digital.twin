    ...
    try:
        for file in files:
            ...
            if os.path.exists(file_path):
                os.remove(file_path)
                logging.info(f"Local file {file} removed.")
            else:
                logging.info(f"Local file {file} does not exist, no need to delete.")
    except Exception as e:
        logging.error(f"Error removing local file: {str(e)}")

# Function to delete .pcap files inside Docker containers
def delete_docker_files():
    try:
        for file in files:
            container_target = containers.get(file)
            if container_target:
                container, target_path = container_target.split(':')
