...
if __name__ == "__main__":
    ...
    for i in range(int(n_slices)):
        ...
        net.addLink(host, s4, bw=1000, delay="1ms", intfName1="upf_" + slices[i]['dnn'] + "-s4", intfName2="s4-upf_" + slices[i]['dnn'])
        # Save the host name for later use
        slices[i]['host_name'] = host_name
        print(f"Saved host_name: {host_name}")
